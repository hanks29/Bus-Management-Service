...
namespace BusManagementService.View
{
    public partial class Drivers : Form
    {
        ...
        private void LoadStudentData()
        {
            ...
            var driverFilter = Builders<Driver>.Filter.Empty;
            var driverList = collection.Find(driverFilter).ToList();
            ...
            foreach (var driver in driverList)
            {
                ...
                foreach (var dailyDriveId in driver.cacChuyenPhuTrach)
                {
                    var dailyDriveFilter = Builders<DailyDrive>.Filter.Eq(dd => dd.dailyDriveId, dailyDriveId);
                    var dailyDrive = dailyDriveCollection.Find(dailyDriveFilter).FirstOrDefault();

                    if (dailyDrive != null)
                    {
                        var routeFilter = Builders<Route>.Filter.Eq(r => r.routeId, dailyDrive.tuyenDuong);
                        var route = routeCollection.Find(routeFilter).FirstOrDefault();

                        if (route != null)
                        {
                            routesList.Add(route.tenTuyenDuong); 
                        }
                    }
                }

                string cacChuyenPhuTrachStr = string.Join(", ", routesList);
        ...
        private void btnsua_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Không tìm thấy tài xế cần sửa", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            if (string.IsNullOrWhiteSpace(txthoten.Text))
            {
        ...
        private void btnxoa_Click(object sender, EventArgs e)
        {
            if(string.IsNullOrWhiteSpace(txtmataixe.Text))
            {
                MessageBox.Show("Không tìm thấy tài xế cần xóa", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }    
            string driverId = txtmataixe.Text;
            var filter = Builders<Driver>.Filter.Eq(d => d.driverId, driverId);
        ...
        private void LoadAllDailyDrives()
        {
            ...
            {
                var route = routeCollection.Find(Builders<Route>.Filter.Eq(r => r.routeId, dailyDrive.tuyenDuong)).FirstOrDefault();

                if (route != null)
                {
                    Chk_listbox.Items.Add(new DailyDriveItem { Text = route.tenTuyenDuong, Value = dailyDrive.dailyDriveId });
                }
            }
        ...
        private void txtsobanglai_KeyPress_1(object sender, KeyPressEventArgs e)
        {
            if (!char.IsControl(e.KeyChar) && !char.IsDigit(e.KeyChar))
            {
                e.Handled = true;
